	...
	private void drawInfoWindow(Canvas canvas, MapView	mapView, boolean shadow) {
		if (null != selected_item && null != selected_item.getTheaterObj()){
			if (!shadow){
				...
				Paint p = new Paint();
				String theater_title = Html.fromHtml(selected_item.getTheaterObj().getTitle()).toString();
				float text_width = p.measureText(theater_title) + 20;
